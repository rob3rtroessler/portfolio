
let phd_awards = [

    // 2016
    {
        name: 'Young Scholar Travel Grant’; Austrian Culture Forum New York',
        type: 'special award',
        value: 400,
        year: "10/01/2016",
        tag: 'travel',
        z: 0
    },
    {
        name: 'Summer Research Award',
        type: 'research award',
        value: 5204,
        year: "07/01/2016",
        tag: 'research',
        z: 0

    },

    // 2017
    {
        name: 'The Bernhard Blume Award for Course Excellence',
        type: 'special award',
        value: 1000,
        year: "05/01/2017",
        tag: 'course work',
        z: 1

    },
    {
        name: 'Summer Research Award',
        type: 'research award',
        value: 5204,
        year: "07/01/2017",
        tag: 'research',
        z: 1

    },
    {
        name: 'The GSAS Predissertation Fellowship',
        type: 'research fellowship',
        value: 5204,
        year: "06/01/2017",
        tag: 'research',
        z: 0
    },

    // 2018
    {
        name: 'Distinction in Teaching Award',
        subtitle: 'Awarded for Fall 2017',
        connections: ['course_01'],
        type: 'teaching award',
        value: 0,
        year: "01/01/2018",
        tag: 'teaching',
        z: 0
    },
    {
        name: 'Summer Research Award',
        type: 'research award',
        value: 5204,
        year: "07/01/2018",
        tag: 'research',
        z: 2

    },
    {
        name: 'Distinction in Teaching Award',
        subtitle: 'for Spring 2018',
        type: 'teaching award',
        value: 0,
        year: "05/01/2018",
        tag: 'teaching',
        z: 1
    },

    // 2019
    {
        name: 'Distinction in Teaching Award for Fall 2018',
        type: 'teaching award',
        value: 0,
        year: "01/01/2019",
        tag: 'teaching',
        z: 2
    },
    {
        name: 'Summer Research Award',
        type: 'research award',
        value: 5204,
        year: "08/01/2019",
        tag: 'research',
        z: 3

    },
    {
        name: 'Distinction in Teaching Award for Spring 2019',
        type: 'teaching award',
        value: 0,
        year: "04/01/2019",
        tag: 'teaching',
        z: 3
    },
    {
        name: 'Pfortzheimer Fellowship',
        type: 'research fellowship',
        value: 5000,
        year: "06/01/2019",
        tag: 'research',
        z: 1
    },

    // 2020
    {
        name: 'Distinction in Teaching Award for Fall 2019',
        type: 'teaching award',
        value: 0,
        year: "01/01/2020",
        tag: 'teaching',
        z: 4
    },
    {
        name: 'Term Merit Stipend',
        type: 'special award',
        value: 14000,
        year: "08/01/2020",
        tag: 'research',
        z: 2
    },
    {
        name: 'Lost Time Funding',
        type: 'other award',
        value: 6000,
        year: "10/01/2020",
        tag: 'covid',
        z: 0
    },

    // 2021
    {
        name: 'Distinction in Teaching Award for Fall 2020',
        type: 'teaching award',
        value: 0,
        year: "01/01/2021",
        tag: 'teaching',
        z: 5
    },
    {
        name: 'Dissertation Completion Fellowship',
        type: 'special award',
        value: 35000,
        year: "09/01/2021",
        tag: 'research',
        z: 3
    },
    {
        name: 'GSAS Support Initiative',
        type: 'other award',
        value: 6000,
        year: "06/01/2021",
        tag: 'covid',
        z: 1
    },
    {
        name: 'Distinction in Teaching Award for Summer 2021',
        type: 'teaching award',
        value: 0,
        year: "11/01/2021",
        tag: 'teaching',
        z: 6
    },

    // 2022
    {
        name: 'Distinction in Teaching Award for Summer 2022',
        type: 'teaching award',
        value: 0,
        year: "09/01/2022",
        tag: 'teaching',
        z: 7
    },

    // 2023
    {
        name: 'Distinction in Teaching Award for Fall 2022',
        type: 'teaching award',
        value: 0,
        year: "01/01/2023",
        tag: 'teaching',
        z: 8
    },
]

let publications = [
    {
        title: 'Vom Dandytum zum Judentum. Biographische und werkästhetische Entwicklungen bei Beer-Hofmann.',
        in: 'In: Tradition in der Wiener Moderne, ed. by Wilhelm Hemecker, Cornelius Mitterer, and David Österle. Berlin, Boston: De Gruyter 2017, pp. 217-234.',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2017',
        z: 0
    },
    {
        title: 'Jung Wien auf der Schulbank.',
        in: 'Co-curated exhibition at the ‘Akademisches Gymnasium’ in Vienna together with PD Dr. Tobias Heinrich (Univ. of Kent)',
        type: 'exhibition',
        subtype: 'paper',
        date: '04/01/2018',
        z: 0
    },
    {
        title: 'Moderne Körperroutinen. Zur Optimierung und Technisierung von Habitus und Haltung in Robert Musils Der Riese Agoag.',
        in: 'In: Das Abenteuer des Gewöhnlichen. Alltag in der deutschsprachigen Literatur der Moderne, ed. By Thorsten Carstensen and Mattias Pirholt, Berlin: Erich Schmidt Verlag 2018, pp. 136 - 152 (=Philologische Studien und Quellen 267) ',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2018',
        z: 0
    },
    {
        title: 'TexTiles: Exploring Patterns in Historical Discourse.',
        in: 'Conference Proceedings of the 4th Workshop on Visualization for the Digital Humanities (at IEEE Vis 2019), pp. 175-281.',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2019',
        z: 0
    },
    {
        title: 'Felix Dörmann als Novellist und Sonntagshumorist.',
        in: 'Felix Dörmann (1870-1928). Special Issue. Journal for Austrian Studies, Volume 54, Number 2, Summer 2021, pp. 47-72.',
        type: 'publication',
        subtype: 'paper',
        date: '07/01/2021',
        z: 0
    },

    {
        title: 'Jung Wien zwischen Stadt und Land. Zu Richard Beer-Hofmanns „sezessionierter“ Villa im Wiener Cottageviertel.',
        in: 'Planen - Wohnen - Schreiben, ed. by Roland Innerhofer et al, Vienna: Picus 2021',
        type: 'publication',
        subtype: 'paper',
        date: '12/01/2021',
        z: 0
    }
]
let projectsAndPapers = [
    {
        title: 'Vom Dandytum zum Judentum. Biographische und werkästhetische Entwicklungen bei Beer-Hofmann.',
        in: 'In: Tradition in der Wiener Moderne, ed. by Wilhelm Hemecker, Cornelius Mitterer, and David Österle. Berlin, Boston: De Gruyter 2017, pp. 217-234.',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2017',
    },
    {
        title: 'Jung Wien auf der Schulbank.',
        in: 'Co-curated exhibition at the ‘Akademisches Gymnasium’ in Vienna together with PD Dr. Tobias Heinrich (Univ. of Kent)',
        type: 'exhibition',
        subtype: 'paper',
        date: '04/01/2018',
    },
    {
        title: 'Moderne Körperroutinen. Zur Optimierung und Technisierung von Habitus und Haltung in Robert Musils Der Riese Agoag.',
        in: 'In: Das Abenteuer des Gewöhnlichen. Alltag in der deutschsprachigen Literatur der Moderne, ed. By Thorsten Carstensen and Mattias Pirholt, Berlin: Erich Schmidt Verlag 2018, pp. 136 - 152 (=Philologische Studien und Quellen 267) ',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2018',
    },
    {
        title: 'TexTiles: Exploring Patterns in Historical Discourse.',
        in: 'Conference Proceedings of the 4th Workshop on Visualization for the Digital Humanities (at IEEE Vis 2019), pp. 175-281.',
        type: 'publication',
        subtype: 'paper',
        date: '09/30/2019',
    },
    {
        title: 'Felix Dörmann als Novellist und Sonntagshumorist.',
        in: 'Felix Dörmann (1870-1928). Special Issue. Journal for Austrian Studies, Volume 54, Number 2, Summer 2021, pp. 47-72.',
        type: 'publication',
        subtype: 'paper',
        date: '07/01/2021',
    },

    {
        title: 'Jung Wien zwischen Stadt und Land. Zu Richard Beer-Hofmanns „sezessionierter“ Villa im Wiener Cottageviertel.',
        in: 'Planen - Wohnen - Schreiben, ed. by Roland Innerhofer et al, Vienna: Picus 2021',
        type: 'publication',
        subtype: 'paper',
        date: '12/01/2021',
    },
    {
        title: 'a',
        type: 'project',
        date: '11/30/2021',
    },
    {
        title: 'a',
        type: 'project',
        date: '11/30/2022',
    },
    {
        title: 'b',
        type: 'project',
        date: '12/13/2022',
    }
]

let presentations = [
    {
        title: 'Urbane Körperkultur und moderne Leistungsethik in Robert Musils Der Riese Agoag',
        info: 'Presentation at the GSA Convention 2016 in San Diego',
        date: '09/30/2016',
        z: 0,
        x_trans: 0

    },
    {
        title: 'Die angeleitete Freizeit. Zur Optimierung der freien Stunden von Arbeitern und Angestellten im frühen 20. Jahrhundert',
        info: 'Talk at the BTWH Conference in Vienna',
        date: '08/21/2017',
        z: 0,
        x_trans: 0
    },
    {
        title: 'Urbane Körperkultur und moderne Leistungsethik in Robert Musils Der Riese Agoag',
        info: 'Presentation at the GSA Convention 2016 in San Diego',
        date: '09/30/2017',
        z: 1,
        x_trans: 0.1
    },
    {
        title: 'Die angeleitete Freizeit. Zur Optimierung der freien Stunden von Arbeitern und Angestellten im frühen 20. Jahrhundert',
        info: 'Talk at the BTWH Conference in Vienna',
        date: '06/21/2017',
        z: 0
    },
    {
        title: 'Urbane Körperkultur und moderne Leistungsethik in Robert Musils Der Riese Agoag',
        info: 'Presentation at the GSA Convention 2016 in San Diego',
        date: '09/30/2018',
        z: 0

    },
    {
        title: 'Die angeleitete Freizeit. Zur Optimierung der freien Stunden von Arbeitern und Angestellten im frühen 20. Jahrhundert',
        info: 'Talk at the BTWH Conference in Vienna',
        date: '06/17/2019',
        z: 0
    },
    {
        title: 'Urbane Körperkultur und moderne Leistungsethik in Robert Musils Der Riese Agoag',
        info: 'Presentation at the GSA Convention 2016 in San Diego',
        date: '06/22/2019',
        z: 1

    },
    {
        title: 'TexTiles: Exploring Patterns in Historical Discourse.',
        info: 'Presentation at the 4th Workshop on Visualization for the Digital Humanities (at IEEE Vis 2019, Vancouver)',
        date: '10/21/2019',
        z: 0
    }
]

let dissertationProgress = [

    {
        date: "09/01/2015",
        pages: 0
    },
    {
        date: "10/01/2015",
        pages: 0
    },
    {
        date: "11/01/2015",
        pages: 0
    },
    {
        date: "12/01/2015",
        pages: 0
    },

    // 2016
    {
        date: "01/01/2016",
        pages: 0
    },
    {
        date: "02/01/2016",
        pages: 0
    },
    {
        date: "03/01/2016",
        pages: 0
    },
    {
        date: "04/01/2016",
        pages: 0
    },
    {
        date: "05/01/2016",
        pages: 0
    },
    {
        date: "06/01/2016",
        pages: 0
    },
    {
        date: "07/01/2016",
        pages: 0
    },
    {
        date: "08/01/2016",
        pages: 0
    },
    {
        date: "09/01/2016",
        pages: 0
    },
    {
        date: "10/01/2016",
        pages: 0
    },
    {
        date: "11/01/2016",
        pages: 0
    },
    {
        date: "12/01/2016",
        pages: 0
    },


    // 2017
    {
        date: "01/01/2017",
        pages: 0
    },
    {
        date: "02/01/2017",
        pages: 0
    },
    {
        date: "03/01/2017",
        pages: 0
    },
    {
        date: "04/01/2017",
        pages: 0
    },
    {
        date: "05/01/2017",
        pages: 0
    },
    {
        date: "06/01/2017",
        pages: 10
    },
    {
        date: "07/01/2017",
        pages: 11
    },
    {
        date: "08/01/2017",
        pages: 15
    },
    {
        date: "09/01/2017",
        pages: 15
    },
    {
        date: "10/01/2017",
        pages: 15
    },
    {
        date: "11/01/2017",
        pages: 15
    },
    {
        date: "12/01/2017",
        pages: 15
    },


    // 2018
    {
        date: "01/01/2018",
        pages: 15
    },
    {
        date: "02/01/2018",
        pages: 15
    },
    {
        date: "03/01/2018",
        pages: 15
    },
    {
        date: "04/01/2018",
        pages: 15
    },
    {
        date: "05/01/2018",
        pages: 15
    },
    {
        date: "06/01/2018",
        pages: 15
    },
    {
        date: "07/01/2018",
        pages: 15
    },
    {
        date: "08/01/2018",
        pages: 15
    },
    {
        date: "09/01/2018",
        pages: 15
    },
    {
        date: "10/01/2018",
        pages: 15
    },
    {
        date: "11/01/2018",
        pages: 15
    },
    {
        date: "12/01/2018",
        pages: 15
    },


    // 2019
    {
        date: "01/01/2019",
        pages: 15
    },
    {
        date: "02/01/2019",
        pages: 15
    },
    {
        date: "03/01/2019",
        pages: 15
    },
    {
        date: "04/01/2019",
        pages: 15
    },
    {
        date: "05/01/2019",
        pages: 15
    },
    {
        date: "06/01/2019",
        pages: 20
    },
    {
        date: "07/01/2019",
        pages: 25
    },
    {
        date: "08/01/2019",
        pages: 28
    },
    {
        date: "09/01/2019",
        pages: 31
    },
    {
        date: "10/01/2019",
        pages: 31
    },
    {
        date: "11/01/2019",
        pages: 31
    },
    {
        date: "12/01/2019",
        pages: 31
    },


    // 2020
    {
        date: "01/01/2020",
        pages: 31
    },
    {
        date: "02/01/2020",
        pages: 31
    },
    {
        date: "03/01/2020",
        pages: 31
    },
    {
        date: "04/01/2020",
        pages: 31
    },
    {
        date: "05/01/2020",
        pages: 31
    },
    {
        date: "06/01/2020",
        pages: 35
    },
    {
        date: "07/01/2020",
        pages: 42
    },
    {
        date: "08/01/2020",
        pages: 48
    },
    {
        date: "09/01/2020",
        pages: 50
    },
    {
        date: "10/01/2020",
        pages: 50
    },
    {
        date: "11/01/2020",
        pages: 50
    },
    {
        date: "12/01/2020",
        pages: 50
    },


    // 2021
    {
        date: "01/01/2021",
        pages: 31
    },
    {
        date: "02/01/2021",
        pages: 31
    },
    {
        date: "03/01/2021",
        pages: 31
    },
    {
        date: "04/01/2021",
        pages: 31
    },
    {
        date: "05/01/2021",
        pages: 31
    },
    {
        date: "06/01/2021",
        pages: 35
    },
    {
        date: "07/01/2021",
        pages: 42
    },
    {
        date: "08/01/2021",
        pages: 48
    },
    {
        date: "09/01/2021",
        pages: 50
    },
    {
        date: "10/01/2021",
        pages: 50
    },
    {
        date: "11/01/2021",
        pages: 50
    },
    {
        date: "12/01/2021",
        pages: 50
    },


    // 2022
    {
        date: "01/01/2022",
        pages: 50
    },
    {
        date: "02/01/2022",
        pages: 50
    },
    {
        date: "03/01/2022",
        pages: 50
    },
    {
        date: "04/01/2022",
        pages: 55
    },
    {
        date: "05/01/2022",
        pages: 70
    },
    {
        date: "06/01/2022",
        pages: 85
    },
    {
        date: "07/01/2022",
        pages: 90
    },
    {
        date: "08/01/2022",
        pages: 95
    },
    {
        date: "09/01/2022",
        pages: 105
    },
    {
        date: "10/01/2022",
        pages: 110
    },
    {
        date: "11/01/2022",
        pages: 110
    },
    {
        date: "12/01/2022",
        pages: 110
    },


    // 2023
    {
        date: "01/01/2023",
        pages: 133
    },
    {
        date: "02/01/2023",
        pages: 160
    },
    {
        date: "03/01/2023",
        pages: 200
    }

]

let skills = [
    {
        skill_abbreviation: "ml",
        skill_long: "Machine Learning",
        rel_start_x: 0.0,
        rel_end_x: 0.16,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "js",
        skill_long: "JavaScript",
        rel_start_x: 0.18,
        rel_end_x: 0.28,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "python",
        skill_long: "Python",
        rel_start_x: 0.3,
        rel_end_x: 0.39,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "react",
        skill_long: "React",
        rel_start_x: 0.41,
        rel_end_x: 0.48,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "stats",
        skill_long: "Statistics",
        rel_start_x: 0.5,
        rel_end_x: 0.57,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "tf",
        skill_long: "TensorFlow",
        rel_start_x: 0.59,
        rel_end_x: 0.72,
        row: 0,
        paradigm: 'cs'
    },

    {
        skill_abbreviation: "pytorch",
        skill_long: "PyTorch",
        rel_start_x: 0.74,
        rel_end_x: 0.85,
        row: 0,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "linguistics",
        skill_long: "linguistics",
        rel_start_x: 0.87,
        rel_end_x: 1,
        row: 0,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "scikit",
        skill_long: "scikit-learn",
        rel_start_x: 0.0,
        rel_end_x: 0.14,
        row: 1,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "pandas",
        skill_long: "pandas",
        rel_start_x: 0.16,
        rel_end_x: 0.25,
        row: 1,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "design",
        skill_long: "Design Thinking",
        rel_start_x: 0.27,
        rel_end_x: 0.4,
        row: 1,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "css",
        skill_long: "CSS",
        rel_start_x: 0.42,
        rel_end_x: 0.49,
        row: 1,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "philosophy",
        skill_long: "Philosophy",
        rel_start_x: 0.51,
        rel_end_x: 0.63,
        row: 1,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "theory",
        skill_long: "Cultural and Literary Theory",
        rel_start_x: 0.65,
        rel_end_x: 0.85,
        row: 1,
        paradigm: 'humanities'
    },

    {
        skill_abbreviation: "datavis",
        skill_long: "Data Visualization",
        rel_start_x: 0.87,
        rel_end_x: 1,
        row: 1,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "html",
        skill_long: "HTML",
        rel_start_x: 0,
        rel_end_x: 0.1,
        row: 2,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "dighum",
        skill_long: "Digital Humanities",
        rel_start_x: 0.12,
        rel_end_x: 0.28,
        row: 2,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "ethics",
        skill_long: "Ethics",
        rel_start_x: 0.3,
        rel_end_x: 0.39,
        row: 2,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "nlp",
        skill_long: "Natural Language Processing",
        rel_start_x: 0.41,
        rel_end_x: 0.61,
        row: 2,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "lithist",
        skill_long: "Literary History",
        rel_start_x: 0.63,
        rel_end_x: 0.75,
        row: 2,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "arthist",
        skill_long: "History of Art and Architecture",
        rel_start_x: 0.77,
        rel_end_x: 1,
        row: 2,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "nodejs",
        skill_long: "node.js",
        rel_start_x: 0,
        rel_end_x: 0.06,
        row: 3,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "psyhist",
        skill_long: "History of Psychology",
        rel_start_x: 0.08,
        rel_end_x: 0.24,
        row: 3,
        paradigm: 'cs'
    },
    {
        skill_abbreviation: "history",
        skill_long: "History",
        rel_start_x: 0.26,
        rel_end_x: 0.34,
        row: 3,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "histsci",
        skill_long: "History of Science",
        rel_start_x: 0.36,
        rel_end_x: 0.50,
        row: 3,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "modernism",
        skill_long: "Literary Modernisms",
        rel_start_x: 0.52,
        rel_end_x: 0.67,
        row: 3,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "medieval",
        skill_long: "Medieval Studies",
        rel_start_x: 0.69,
        rel_end_x: 0.81,
        row: 3,
        paradigm: 'humanities'
    },
    {
        skill_abbreviation: "ai",
        skill_long: "Artificial Intelligence",
        rel_start_x: 0.83,
        rel_end_x: 1,
        row: 3,
        paradigm: 'cs'
    }
]

let awardTypeColorLookup = {
    "annual award": "#8dd3c7",
    "teaching award": "#ffffb3",
    "other award": "#bebada",
    "special award": "#fb8072",
    "research fellowship": "#80b1d3",
    "research award": "#fdb462"
}

let colorCourseLookupTable = {
    arthist: "#E29578",
    css: "#006D77",
    datavis: "#006D77",
    design: "#006D77",
    dighum: "#006D77",
    discourse: "#E29578",
    ethics: "#E29578",
    history: "#E29578",
    histsci: "#E29578",
    html: "#006D77",
    js: "#006D77",
    lithist: "#E29578",
    linguistics: "#E29578",
    pandas:"#006D77",
    philosophy: "#E29578",
    python: "#006D77",
    pytorch: "#006D77",
    psyhist: "#E29578",
    medieval: "#E29578",
    ml: "#006D77",
    modernism: "#E29578",
    nlp: "#006D77",
    nodejs: "#006D77",
    react: "#006D77",
    scikit: "#006D77",
    sql: "#006D77",
    stats: "#006D77",
    theory: "#E29578",
    tf: "#006D77"
}

let shortTolongSkill = {
    arthist: "Art&nbsp;History",
    css: "CSS",
    datavis: "Data&nbsp;Visualization",
    design: "Design&nbsp;Thinking",
    dighum: "Digital&nbsp;Humanities",
    ethics: "Ethics",
    history: "History",
    histsci: "History&nbsp;of&nbsp;Science",
    html: "HTML",
    js: "JavaScript",
    linguistics: "Linguistics",
    lithist: "Literary&nbsp;History",
    medieval: "Medieval Studies",
    ml: "Machine&nbsp;Learning",
    modernism: "Literary Modernisms",
    nlp: "Natural&nbsp;Language&nbsp;Processing",
    nodejs: "node.js",
    pandas: "Pandas",
    philosophy: "Philosophy",
    python: "Python",
    pytorch: "PyTorch",
    react: "React",
    sql: "SQL",
    scikit: "scikit-learn",
    stats: "statistics",
    theory: "Literary&nbsp;Theory",
    tf: "TensorFlow",
}

let gradeColorLookUpTable = {
    "A": "#238443",
    "A-": "#41ab5d",
    "B+": "#78c679",
    "B": "#addd8e",
    "B-": "#d9f0a3",
    "N/A": '#f7fcb9'
}




let paradigmColorLookUpTable = {
    cs: '#006D77',
    humanities: '#E29578'
}